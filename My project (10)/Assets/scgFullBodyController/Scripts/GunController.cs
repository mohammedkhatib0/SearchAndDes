...
namespace scgFullBodyController
{
    ...
    public class GunController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if ((Input.GetKey(KeyCode.R) )&& !reloading && !firing && bulletsInMag < bulletsPerMag && totalBullets > 0)
            {
                anim.SetBool("reload", true);
                reloading = true;
                //gameObject.GetComponent<AudioSource>().PlayOneShot(reloadSound);
                gunPV.RPC("ReloadSound", RpcTarget.All);
        ...
        void spawnBullet()
        {
            if (shootFromCamera)
            {
                ...
                if (PV.IsMine)
                {
                    Vector3 pos = shootPointCamera.transform.position;
                    gunPV.RPC("Shoot", RpcTarget.All, pos);
                }
            }
            else
            {
        ...
        {
            tempBullet = tempBullet = Instantiate(Bullet, hitPoint, Quaternion.identity) as GameObject;
        ...
        void spawnShell()
        {
            ...
            if (PV.IsMine)
            {
                tempShell = PhotonNetwork.Instantiate(Path.Combine("PhotonPrefabs", "Shell"), ejectionPoint.transform.position, ejectionPoint.transform.rotation) as GameObject;
           ...
           //// PhotonNetwork.Destroy(tempShell, shellDespawnTime);
        }
        ...
        void spawnMag()
        {
            ...
            if (PV.IsMine)
            {
                tempMag = PhotonNetwork.Instantiate(Path.Combine("PhotonPrefabs", "Mag"), magDropPoint.transform.position, magDropPoint.transform.rotation) as GameObject;
            }
            else return;
